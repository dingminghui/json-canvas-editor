...
当前项目已将该原则落实为自动配图门禁：模型先判断页面是否需要图片，仅为适合配图且没有结构化主视觉的页面生成检索需求；系统读取 Pexels 第 1 页候选，模型自动选择单张素材。可选图片失败时回退到原生排版，不允许用手动上传或人工挑图绕过内容与版式判断。

质量门禁同时检查演示字号下限和有效视觉占比。内容不够时应合并、补足或明确失败，不能通过缩小字体、把内容压在页面局部或加入无关装饰图来制造“完成”的假象。首轮视觉评审若产生修订，重渲染结果还必须再通过一次最终看图验证。
